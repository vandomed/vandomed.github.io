...
# Run 1,000 trials for each scenario (might take about an hour to run)
set.seed(123)
...
p <- ggplot(df2, aes(x = days, y = mbhat)) + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        axis.title.x = element_text(vjust = -1)) + 
  scale_x_discrete("Number of days sampled", limits = 1: 7) + 
...
# Run 1000 trials for each scenario (might take 5-6 hours to run)
set.seed(1234)
betas <- f.resampling %>% iterate(sex = 1: 2, days = 1: 7, trials = 2)
betas$sex <- factor(betas$sex, levels = c(1, 2), labels = c("Males", "Females"))
...
df2$Variable <- factor(df2$var, levels = as.character(unique(df2$var)), labels = varlabs)
df2$outcome <- c(rep("HDL", nrow(df2) / 2), rep("log(CRP)", nrow(df2) / 2))

p <- ggplot(df2, aes(x = days, y = atten, group = Variable, 
                     color = Variable, linetype = Variable)) + 
  geom_point(size = 1) +
  geom_line(lwd = 0.5) + 
  scale_color_manual(values = c(rep(brewer.pal(n = 6, name = "Dark2"), 2), "black")) + 
  scale_linetype_manual(values = c(rep("solid", 7), rep("dotted", 6))) + 
  geom_hline(yintercept = 0, lty = 2, lwd = 0.5) + 
  facet_grid(outcome ~ sex) + 
  labs(title = "Percent Attenuation vs. Number of Days Sampled", 
       y = "Percent attenuation", 
       x = "Number of days sampled") + 
  scale_x_discrete("Days sampled", limits = 1: 7) + 
  scale_y_continuous("Percent attenuation", limits = c(-27, 75), breaks = seq(-25, 75, 25)) + 
